  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {hasErrors && (
          <div
            className="w-full block border border-destructive/20 bg-destructive/10
            rounded text-sm max-h-60 overflow-y-auto"
            style={{
              maxHeight: "250px",
            }}
          >
            <p className="font-medium mb-2 bg-destructive/10 sticky top-0 px-2 py-1">
              Issues found:
            </p>
            <div className="space-y-1 p-2">
              {Object.entries(errors).map(([row, msg]) => (
                <details key={row} className="group">
                  <summary className="flex text-sm items-center justify-between cursor-pointer !text-destructive">
                    <span>Row {row}</span>
                    <ChevronDown className="w-4 h-4 transform group-open:rotate-180 transition-transform" />
                  </summary>
                  <div className="mt-1 pl-2 text-xs !text-destructive/80 border-l-2 border-destructive/40">
                    {msg.split("\n").map((line, i) => (
                      <p key={i}>{line}</p>
                    ))}
                  </div>
